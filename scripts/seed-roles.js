...
async function seedRoles() {
  ...
  try {
    ...
    // Seed Sample Clinics if table exists
    try {
      let clinicTable = null;
      try {
        await client.query('SELECT 1 FROM "clinic"."Clinic" LIMIT 1');
        clinicTable = '"clinic"."Clinic"';
      } catch {
        try {
          await client.query('SELECT 1 FROM "public"."Clinic" LIMIT 1');
          clinicTable = '"public"."Clinic"';
        } catch {}
      }

      if (clinicTable) {
        const checkClinics = await client.query(`SELECT COUNT(*) as count FROM ${clinicTable}`);
        if (Number(checkClinics.rows[0]?.count || 0) === 0) {
          const sampleClinics = [
            { name: 'MedCare Central Medical Center', address: '742 Evergreen Terrace, Suite 100', city: 'Metropolis', phone: '+1 (555) 234-5678', email: 'central@medcare.com' },
            { name: 'Downtown Specialist & Urgent Care', address: '120 Market St, Floor 3', city: 'Metropolis', phone: '+1 (555) 345-6789', email: 'downtown@medcare.com' },
            { name: 'Metro Family & Pediatric Clinic', address: '450 Oak Ave', city: 'Metropolis', phone: '+1 (555) 456-7890', email: 'family@medcare.com' },
          ];
          const now = new Date();
          for (const c of sampleClinics) {
            await client.query(
              `INSERT INTO ${clinicTable} ("id", "name", "address", "city", "phone", "email", "status", "createdAt", "updatedAt")
               VALUES ($1, $2, $3, $4, $5, $6, 'ACTIVE', $7, $8)`,
              [randomUUID(), c.name, c.address, c.city, c.phone, c.email, now, now]
            );
          }
          console.log('    🏢 Seeded default sample clinics.');
        }
      }
    } catch (err) {
      console.warn(`    ⚠️ Notice seeding clinics: ${err.message}`);
    }

    // Seed Sample Appointments if table exists
    try {
      let apptTable = null;
      try {
        await client.query('SELECT 1 FROM "appointment"."Appointment" LIMIT 1');
        apptTable = '"appointment"."Appointment"';
      } catch {
        try {
          await client.query('SELECT 1 FROM "public"."Appointment" LIMIT 1');
          apptTable = '"public"."Appointment"';
        } catch {}
      }

      if (apptTable) {
        const checkAppts = await client.query(`SELECT COUNT(*) as count FROM ${apptTable}`);
        if (Number(checkAppts.rows[0]?.count || 0) === 0) {
          const now = new Date();
          const todayStr = now.toISOString().split('T')[0];
          const sampleAppts = [
            { patientName: 'John Doe', doctorName: 'Dr. Sarah Jenkins', time: '09:00 AM', status: 'CONFIRMED', reason: 'Routine Cardiology Follow-up', fee: 75 },
            { patientName: 'Robert Chen', doctorName: 'Dr. Sarah Jenkins', time: '10:30 AM', status: 'IN_PROGRESS', reason: 'Blood Pressure Evaluation', fee: 75 },
            { patientName: 'Emily Watson', doctorName: 'Dr. Sarah Jenkins', time: '01:00 PM', status: 'CONFIRMED', reason: 'Annual Heart Health Check', fee: 75 },
            { patientName: 'David Kim', doctorName: 'Dr. Sarah Jenkins', time: '02:30 PM', status: 'COMPLETED', reason: 'ECG Review & Consultation', fee: 75 },
            { patientName: 'Maria Garcia', doctorName: 'Dr. Sarah Jenkins', time: '04:00 PM', status: 'SCHEDULED', reason: 'Palpitations Assessment', fee: 75 },
          ];

          for (const a of sampleAppts) {
            await client.query(
              `INSERT INTO ${apptTable} ("id", "appointmentNumber", "patientId", "patientName", "doctorId", "doctorName", "date", "time", "type", "status", "reason", "fee", "isPaid", "createdAt", "updatedAt")
               VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'IN_PERSON', $9, $10, $11, true, $12, $13)`,
              [
                randomUUID(),
          ...
          console.log('    📅 Seeded default sample appointments.');
        }
      }
    } catch (err) {
      console.warn(`    ⚠️ Notice seeding appointments: ${err.message}`);
    }
