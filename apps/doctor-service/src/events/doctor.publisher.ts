...
export class DoctorEventPublisher {
  ...
  async publishVerificationSubmitted(doctorId: string, payload: any) {
    this.logger.log(
      `Publishing event ${EVENTS.DOCTOR.VERIFICATION_SUBMITTED} for doctor ${doctorId}`,
    );
    await this.kafka.emit(EVENTS.DOCTOR.VERIFICATION_SUBMITTED, {
      doctorId,
      ...payload,
    });
  }
  ...
  async publishPayoutRequested(doctorId: string, amount: number) {
    this.logger.log(
      `Publishing event ${EVENTS.DOCTOR.PAYOUT_REQUESTED} for doctor ${doctorId}`,
    );
