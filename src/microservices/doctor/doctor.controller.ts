...
export class DoctorController {
  ...
  async updateStatus(@Payload() payload: { id: string; dto: UpdateDoctorStatusDto; actorId?: string }) {
    return this.doctorService.updateDoctorStatus(payload.id, payload.dto.status, payload.dto.reason);
  }
  ...
  async patientGetSlots(@Payload() payload: { doctorId: string; date: string }) {
    return this.doctorService.patientGetAvailableSlots(payload.doctorId, payload.date);
  }
  ...
  async doctorGetWorkspace(@Payload() payload: { userId: string; appointmentId: string }) {
    return this.doctorService.doctorGetConsultationWorkspace(payload.userId, payload.appointmentId);
  }

  @MessagePattern(PATTERNS.DOCTOR.DOCTOR_SAVE_NOTES)
  async doctorSaveNotes(@Payload() payload: { userId: string; appointmentId: string; dto: SaveConsultationNotesDto }) {
    return this.doctorService.doctorSaveConsultationNotes(payload.userId, payload.appointmentId, payload.dto);
  }

  @MessagePattern(PATTERNS.DOCTOR.DOCTOR_COMPLETE_CONSULTATION)
  async doctorCompleteConsultation(@Payload() payload: { userId: string; appointmentId: string }) {
    return this.doctorService.doctorCompleteConsultation(payload.userId, payload.appointmentId);
  }

  @MessagePattern(PATTERNS.DOCTOR.DOCTOR_CREATE_PRESCRIPTION)
  async doctorCreatePrescription(@Payload() payload: { userId: string; dto: CreateDoctorPrescriptionDto }) {
    return this.doctorService.doctorCreatePrescription(payload.userId, payload.dto);
  }

  @MessagePattern(PATTERNS.DOCTOR.DOCTOR_LIST_PRESCRIPTIONS)
  async doctorListPrescriptions(@Payload() payload: { userId: string; query: any }) {
    return this.doctorService.doctorListPrescriptions(payload.userId, payload.query);
  }

  @MessagePattern(PATTERNS.DOCTOR.DOCTOR_GET_PRESCRIPTION)
  async doctorGetPrescription(@Payload() payload: { userId: string; id: string }) {
    return this.doctorService.doctorGetPrescriptionDetails(payload.userId, payload.id);
  }

  @MessagePattern(PATTERNS.DOCTOR.DOCTOR_LIST_APPOINTMENTS)
  async doctorListAppointments(@Payload() payload: { userId: string; query: DoctorAppointmentFilterDto }) {
    return this.doctorService.doctorListAppointments(payload.userId, payload.query);
  }

  @MessagePattern(PATTERNS.DOCTOR.DOCTOR_GET_APPOINTMENT)
  async doctorGetAppointment(@Payload() payload: { userId: string; id: string }) {
    return this.doctorService.doctorGetAppointmentDetails(payload.userId, payload.id);
  }

  @MessagePattern(PATTERNS.DOCTOR.DOCTOR_GET_VIDEO_TOKEN)
  async doctorGetVideoToken(@Payload() payload: { userId: string; appointmentId: string }) {
    return this.doctorService.doctorGetVideoSessionToken(payload.userId, payload.appointmentId);
  }
  ...
  async doctorGetPatientRecords(@Payload() payload: { userId: string; patientId: string }) {
    return this.doctorService.doctorGetPatientMedicalRecords(payload.userId, payload.patientId);
  }
  ...
  async doctorUpdateSchedule(@Payload() payload: { userId: string; dto: DoctorScheduleDto }) {
  ...
  async doctorRequestPayout(@Payload() payload: { userId: string; dto: DoctorPayoutRequestDto }) {
  ...
  async doctorReplyReview(@Payload() payload: { userId: string; reviewId: string; reply: string }) {
    return this.doctorService.doctorReplyReview(payload.userId, payload.reviewId, payload.reply);
  }
  ...
  async doctorUpdateProfile(@Payload() payload: { userId: string; dto: UpdateDoctorProfileDto }) {
