...
describe('DoctorGatewayController', () => {
  ...
  const mockUser = { id: 'doc-user-1', email: 'doctor@medcare.local', role: UserRole.DOCTOR };
  ...
  beforeEach(async () => {
    doctorService = {
      ...
      doctorSaveConsultationNotes: jest.fn().mockResolvedValue({ id: 'note-1' }),
      doctorCompleteConsultation: jest.fn().mockResolvedValue({ success: true }),
      doctorCreatePrescription: jest.fn().mockResolvedValue({ id: 'rx-1' }),
      doctorListPrescriptions: jest.fn().mockResolvedValue({ items: [], meta: { total: 0 } }),
      doctorGetPrescriptionDetails: jest.fn().mockResolvedValue({ id: 'rx-1' }),
      doctorListAppointments: jest.fn().mockResolvedValue({ items: [], meta: { total: 0 } }),
      doctorGetAppointmentDetails: jest.fn().mockResolvedValue({ id: 'apt-1' }),
      doctorGetVideoSessionToken: jest.fn().mockResolvedValue({ channelName: 'medcare-call-101', token: 'mock-tok' }),
      doctorListPatients: jest.fn().mockResolvedValue({ items: [], meta: { total: 0 } }),
      doctorGetPatientMedicalRecords: jest.fn().mockResolvedValue([]),
      doctorGetSchedule: jest.fn().mockResolvedValue({ consultationFee: 150, schedules: [] }),
      doctorUpdateSchedule: jest.fn().mockResolvedValue({ consultationFee: 150, schedules: [] }),
      doctorGetEarningsSummary: jest.fn().mockResolvedValue({ kpi: { today: 150 } }),
      doctorRequestPayout: jest.fn().mockResolvedValue({ id: 'payout-1' }),
      doctorListReviews: jest.fn().mockResolvedValue({ reviews: [], meta: { total: 0 } }),
  ...
  it('should get consultation workspace', async () => {
    ...
    expect(res).toBeDefined();
    expect(doctorService.doctorGetConsultationWorkspace).toHaveBeenCalledWith('doc-user-1', 'apt-1');
  });
  ...
  it('should save consultation notes', async () => {
    ...
    expect(res).toBeDefined();
    expect(doctorService.doctorSaveConsultationNotes).toHaveBeenCalledWith('doc-user-1', 'apt-1', expect.anything());
  });

  it('should complete consultation', async () => {
    const res = await controller.completeConsultation(mockReq, 'apt-1');
    expect(res).toBeDefined();
    expect(doctorService.doctorCompleteConsultation).toHaveBeenCalledWith('doc-user-1', 'apt-1');
  });
  ...
  it('should create digital prescription', async () => {
    ...
    expect(res).toBeDefined();
    expect(doctorService.doctorCreatePrescription).toHaveBeenCalledWith('doc-user-1', expect.anything());
  });

  it('should get video session token', async () => {
    const res = await controller.getVideoSessionToken(mockReq, 'apt-1');
    expect(res).toBeDefined();
    expect(doctorService.doctorGetVideoSessionToken).toHaveBeenCalledWith('doc-user-1', 'apt-1');
  });
  ...
  it('should get and update schedule', async () => {
    ...
    expect(doctorService.doctorUpdateSchedule).toHaveBeenCalledWith('doc-user-1', expect.anything());
  });
  ...
  it('should get earnings and request payout', async () => {
    ...
    expect(doctorService.doctorRequestPayout).toHaveBeenCalledWith('doc-user-1', expect.anything());
  });
});
