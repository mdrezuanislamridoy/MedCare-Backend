...
export class SupportStaffGatewayController {
  ...
  // 1. DASHBOARD & KPIS
  // ==========================================
  @ApiOperation({ summary: 'Get support staff dashboard overview, KPIs, and urgent ticket queues' })
  ...
  // 2. SUPPORT TICKETS
  // ==========================================
  @ApiOperation({ summary: 'List, search, and filter support tickets by status, priority, and category' })
  ...
  @ApiOperation({ summary: 'Get full conversation thread and internal staff notes for a ticket' })
  ...
  @ApiOperation({ summary: 'Create a new support ticket on behalf of a patient' })
  ...
  @ApiOperation({ summary: 'Send public reply or append internal note to ticket thread' })
  ...
  @ApiOperation({ summary: 'Assign or reassign ticket ownership to a support agent' })
  ...
  @ApiOperation({ summary: 'Transition ticket status (IN_PROGRESS, WAITING_FOR_USER, RESOLVED, CLOSED)' })
  ...
  @ApiOperation({ summary: 'Get complaint investigation details and doctor references' })
  ...
  @ApiOperation({ summary: 'Update complaint status (UNDER_INVESTIGATION, RESPONDED, RESOLVED)' })
  ...
  @ApiOperation({ summary: 'Escalate severe patient complaint to Clinic Management / Super Admin' })
  ...
  // ==========================================
  @ApiOperation({ summary: 'Search patients by name, email, phone, or ID (excludes EHR/medical history)' })
  @ApiResponse({ status: 200, description: 'Patient directory results returned' })
  @Get('patients')
  async searchPatients(@Query() query: SupportPatientSearchDto) {
    return this.supportService.searchPatients(query);
  }

  @ApiOperation({ summary: 'Trigger resend of patient verification email or appointment SMS' })
  ...
  async resendPatientNotification(
    ...
    return this.supportService.resendPatientNotification(id, body.type, req.user.id);
  }

  // ==========================================
  // 5. APPOINTMENT ISSUE ASSISTANCE
  // ==========================================
  @ApiOperation({ summary: 'List appointments flagged with patient issues or requiring assistance' })
  ...
  @ApiOperation({ summary: 'Assist patient in rescheduling an appointment to a new date/time or doctor' })
  ...
  async assistRescheduleAppointment(
    ...
    return this.supportService.assistRescheduleAppointment(id, body, req.user.id);
  }

  @ApiOperation({ summary: 'Mark appointment issue resolved and clear issue flag' })
  ...
  // ==========================================
  @ApiOperation({ summary: 'List immutable support staff action and activity audit logs' })
