...
export class ClinicManagerGatewayController {
  ...
  // ==========================================
  @ApiOperation({
    summary: 'Get clinic branch overview metrics (doctors on duty, occupied rooms, queue, daily revenue)',
  })
  @ApiResponse({ status: 200, description: 'Dashboard metrics returned' })
  @ApiQuery({ name: 'clinicId', required: false, description: 'Optional clinic branch ID for Admins' })
  @Get('dashboard')
  async getDashboard(
  ...
  @ApiOperation({ summary: 'Update clinic branch contact details, address, and operating info' })
  ...
  async updateProfile(
    ...
    return this.clinicService.updateClinicProfile(req.user.id, clinicId, body, req.user.id);
  }

  // ==========================================
  // 3. DOCTORS MANAGEMENT
  // ==========================================
  @ApiOperation({ summary: 'List doctors assigned to this clinic with duty schedule and room' })
  ...
  @ApiOperation({ summary: 'Assign a doctor to this clinic branch and allocate room' })
  ...
  async assignDoctor(
    ...
    return this.clinicService.assignDoctorToClinic(req.user.id, clinicId, body, req.user.id);
  }
  ...
  async removeDoctor(
    ...
    return this.clinicService.removeDoctorFromClinic(req.user.id, clinicId, doctorId, req.user.id);
  }

  // ==========================================
  // 4. STAFF ROSTER MANAGEMENT
  // ==========================================
  @ApiOperation({ summary: 'List clinic staff roster (receptionists, nurses, lab techs) and shift status' })
  ...
  @ApiOperation({ summary: 'Add a new staff member to the clinic branch roster' })
  ...
  async createStaff(
    ...
    return this.clinicService.createClinicStaff(req.user.id, clinicId, body, req.user.id);
  }

  @ApiOperation({ summary: 'Update staff shift status, shift hours, or assigned department' })
  ...
  async updateStaff(
    ...
    return this.clinicService.updateClinicStaff(req.user.id, clinicId, staffId, body, req.user.id);
  }
  ...
  async deleteStaff(
    ...
    return this.clinicService.deleteClinicStaff(req.user.id, clinicId, staffId, req.user.id);
  }

  // ==========================================
  // 5. ROOMS MANAGEMENT
  // ==========================================
  @ApiOperation({ summary: 'List clinic consultation and surgical rooms with live occupancy status' })
  ...
  @ApiOperation({ summary: 'Create a new room in the clinic with equipment and doctor assignment' })
  ...
  async createRoom(
    ...
    return this.clinicService.createClinicRoom(req.user.id, clinicId, body, req.user.id);
  }

  @ApiOperation({ summary: 'Update room status (AVAILABLE, OCCUPIED, MAINTENANCE) or assigned doctor' })
  ...
  async updateRoom(
    ...
    return this.clinicService.updateClinicRoom(req.user.id, clinicId, roomId, body, req.user.id);
  }
  ...
  async deleteRoom(
    ...
    return this.clinicService.deleteClinicRoom(req.user.id, clinicId, roomId, req.user.id);
  }

  // ==========================================
  // 6. APPOINTMENTS & QUEUES
  // ==========================================
  @ApiOperation({ summary: 'List clinic branch appointments with patient and doctor details' })
  ...
  async getAppointments(
    ...
    return this.clinicService.getClinicAppointments(req.user.id, clinicId, filter);
  }

  @ApiOperation({ summary: 'Get real-time patient queue for the clinic branch' })
  ...
  // ==========================================
  @ApiOperation({ summary: 'Get clinic branch revenue breakdown (cash, video visits, completed consultations)' })
  ...
  async getPayments(
    ...
    return this.clinicService.getClinicFinancialSummary(req.user.id, clinicId, filter);
  }

  @ApiOperation({ summary: 'Get clinic performance analytics, completion rates, and doctor utilization' })
  ...
  // ==========================================
  @ApiOperation({ summary: 'Get clinic branch audit trail and operational activity logs' })
  ...
  async getActivityLogs(
    ...
    return this.clinicService.getClinicActivityLogs(req.user.id, clinicId, { page, limit });
  }

  // ==========================================
  // 9. LIVE ROOM & QUEUE SSE STREAM
  // ==========================================
  @ApiOperation({ summary: 'Server-Sent Events (SSE) live stream for room occupancy and queue updates' })
