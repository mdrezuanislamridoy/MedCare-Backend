...
export class StandaloneReviewAppModule {}

async function bootstrap() {
  const options = getMicroserviceServerOptions(
    parseInt(process.env.REVIEW_SERVICE_PORT || '3011', 10),
  );
  const app = await NestFactory.createMicroservice<MicroserviceOptions>(
    StandaloneReviewAppModule,
    options,
  );
  const config = getTransportConfig();
  await app.listen();
  console.log(
    `⭐ MedCare Review & Ratings Microservice is running [Transport: ${config.transportType}]`,
  );
}
